...
\begin{itemize}
  \item \texttt{eqs}, a list or tuple of \texttt{Equation} objects. A function
...
\end{lstlisting}
where \texttt{X} is a list or tuple of scalar being solved for (for example
each as returned by the \texttt{new\_scalar\_function} function -- see section
...
eq = PointInterpolationSolver(y, X, X_coords=X_coords)
\end{lstlisting}
where \texttt{X} is a list or tuple of scalars being solved for (for
example each as returned by the \texttt{new\_scalar\_function} function -- see
