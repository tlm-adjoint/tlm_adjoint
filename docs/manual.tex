...
\begin{itemize}
  ...
  \item \texttt{nonzero\_initial\_guess}, a logical, whether to use a non-zero
    initial guess for the solution to the final \texttt{Equation} in
    \texttt{eqs} (default true).
...
\begin{itemize}
  ...
  \item \texttt{initial\_guess}, the initial guess used in a forward solve.
...
\begin{itemize}
  ...
  \item \texttt{ic\_deps}, a list or tuple of functions, defining the initial
    condition dependencies of the equation. If not supplied then set equal to a
    list containing \texttt{x} if it is in
    \texttt{nl\_deps}, or an empty list otherwise.
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{b}, a function, defining the right-hand-side of the linear
    system ($b$ above). May be modified.
\end{itemize}
This method returns a function defining the solution of the linear system
($\lambda_x$ above), or may return \texttt{None} if this is zero. The return
value will not be modified by calling code.
...
always call the base class \texttt{replace} method.
...
\begin{itemize}
  ...
  \item \texttt{has\_ic\_dep}. If true then, in solving a linear equation
    involving the matrix, the initial value of the solution variable is an
    initial condition dependency of the equation.
\end{itemize}
...
\end{lstlisting}
where the \texttt{nl\_deps} and \texttt{b} arguments are as in the
\texttt{forward\_solve} method. Returns the result as a function. The return
value will not be modified by calling code.
...
A \texttt{Matrix} may define the methods
\begin{lstlisting}
def reset_adjoint(self):
\end{lstlisting}
\begin{lstlisting}
...
def finalize_adjoint(self, J):
\end{lstlisting}
These are called by the respective methods of a \texttt{LinearEquation} using
the \texttt{Matrix}.
...
functions. The return value of \texttt{forward\_solve} and
\texttt{adjoint\_solve} should also be list or tuple of functions. The
\texttt{b} argument of \texttt{adjoint\_action} is equal to
\texttt{B[b\_index]}.
...
\ref{sect:higher_order_adjoint}). The optional \texttt{callback} argument takes
the form
\begin{lstlisting}
