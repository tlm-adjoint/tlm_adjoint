...
\begin{itemize}
  ...
  \item \texttt{B\_action}, a callable which accepts a single input function,
    ...
    be supplied if \texttt{action\_type} equals ``conjugate\_dual''.
  \item \texttt{space\_type}, ``primal'', ``conjugate\_primal'', ``dual'', or
    ``conjugate\_dual'', defining the space type for eigenvectors.
  \item \texttt{action\_type}, ``primal'' or ``conjugate\_dual'', whether the
    action is in the same space as the eigenvectors, or the associated
...
\end{lstlisting}
where \texttt{m} is a function defining the control.
