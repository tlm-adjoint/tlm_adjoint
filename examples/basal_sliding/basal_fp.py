...
def forward(beta_sq, ref = None, h_filename = None, speed_filename = None):
  ...
  def momentum(U, h, initial_guess = None):  
    ...
    class ArrayPowerEquation(Equation):
      def __init__(self, c, n, y, x, z = None):
        Equation.__init__(self, x, deps = [x, y] if z is None else [x, y, z], nl_deps = [y] if z is None else [y, z])
        self._c = c
        self._n = n
        self._z = z
      ...
      def forward_solve(self, x, deps = None):
        if self._z is None:
          ...
          function_set_values(x, self._c * numpy.power(y, self._n))
        else:
          _, y, z = self.dependencies() if deps is None else deps
          y = function_get_values(y)
          z = function_get_values(z)
          function_set_values(x, self._c * numpy.power(y, self._n) * z)
      ...
      def adjoint_derivative_action(self, nl_deps, dep_index, adj_x):
        if dep_index == 0:
          ...
          adj_x = function_get_values(adj_x)
          if self._z is None:
            y, = nl_deps
            y = function_get_values(y)
            function_set_values(F, numpy.power(y, self._n - 1) * adj_x)
          else:
            y, z = nl_deps
            y = function_get_values(y)
            z = function_get_values(z)
            function_set_values(F, numpy.power(y, self._n - 1) * adj_x * z)
          return (-self._n * self._c, F)
        elif dep_index == 2 and not self._z is None:
          F = function_new(self.dependencies()[dep_index])
          adj_x = function_get_values(adj_x)
          y, _ = nl_deps
          y = function_get_values(y)
          function_set_values(F, numpy.power(y, self._n) * adj_x)
          return (-self._c, F)
        else:
          return None
      ...
      def tangent_linear(self, M, dM, tlm_map):
        ...
        if tau_y is None:
          ...
          return ArrayPowerEquation(self._n * self._c, self._n - 1, y, tlm_map[x], z = tau_y)    
    nu_eq = ArrayPowerEquation(0.5 * float(B), (1.0 - float(n)) / (2.0 * float(n)), S, nu)
